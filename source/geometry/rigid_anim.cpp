...
namespace RigidAnims {
    ...
    bool CRigidAnimResource::create(const std::string& name) {
        ...
        const std::vector<char> &data = EngineFiles.loadResourceFile(name);
        int file_pos = 0;
        ...
        memcpy_s(&tracks[0], tracks.size() * sizeof(TTrackInfo), &data[file_pos], tracks.size() * sizeof(TTrackInfo));
        file_pos += tracks.size() * sizeof(TTrackInfo);
        //fdp.readBytes(tracks.data(), tracks.size() * sizeof(TTrackInfo));

        keys.resize(header.num_keys);
        memcpy_s(&keys[0], keys.size() * sizeof(TKey), &data[file_pos], keys.size() * sizeof(TKey));
        file_pos += keys.size() * sizeof(TKey);
